...
class Pay
{
 ...
 //      		{
            return Pay::zfbjk_html($user);
                //  return Pay::zfbjk_html($user).Pay::wxpay_html($user);
//		}
            default:
            return "";
        }
...
public static function getALIFACETOFACE($user) {
    if(Config::get('alifacetofaceopen') =='true') {

        return Pay::alipay_html($user);
    }
    return '';
...
public static function getWXFACETOFACE($user) {
  if(Config::get('wxfacetofaceopen') =='true') {

    return Pay::wxpay_html($user);
}
return '';

} 
private static function alipay_html($user)
{
    return '
    <p class="card-heading">支付宝（推荐）</p>
    <p>按照下面的步骤，转账成功后，1分钟内自动到账。<a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=4805062&Site=&Menu=yes">充值不到账？点击联系客服</a><p>
        <p>打开支付宝，扫描以下二维码进行转账，备注（留言）请填写:  <code>'.$user->id.'</code>（留言/备注前面数字，<font color="#FF0000">只需填写数字</font>，必须填写备注，不填写备注或备注填写错误，无法到账）。<br></p>
        <img src="'.Config::get('ali_facetofacemoney').'"/>
        ';
    }

    private static function wxpay_html($user)
    {
        return '
        <p class="card-heading">微信支付（推荐）</p>
        <p>按照下面的步骤，转账成功后，1分钟内自动到账。<a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=4805062&Site=&Menu=yes">充值不到账？点击联系客服</a><p>
            <p>打开微信，扫描以下二维码进行转账，备注（留言）请填写:  <code>'.$user->id.'</code>（留言/备注前面数字，<font color="#FF0000">只需填写数字</font>，必须填写备注，不填写备注或备注填写错误，无法到账）。<br></p>
            <img src="'.Config::get('wx_facetofacemoney').'"/>
            ';
        }    
    ...
    private static function golangwx_callback($request) {
	$alidirect_pid = Config::get("golangwx_id"); //自己写的微信抓单的内容不需要写其他的。  用来做md5 加密    
     	/*$tradeNo =$_POST['TradeNo'];
       $Money = $_POST['Money'];
        $title = $_POSTi['Title'];
        $userid =$_POST['Userid'];
        $Sign = $_POST['Sign'];
	$_POST['123123'] = "123123123123";
	var_dump($_POST);
	...
	*/
    ...
    $Sign = $request->getParam('Sign');
	//$s1 =strtoupper(md5($alidirect_pid . $tradeNo . $Money . $title .$userid));
	//$s2 = ($alidirect_pid . $tradeNo . $Money . $title .$userid);
	//echo md5("1");
        //echo $s1."<><>><><><><><>".$s2;
    if (strtoupper(md5($alidirect_pid . $tradeNo . $Money . $title .$userid)) == strtoupper($Sign)) {
        $trade = Paylist::where("tradeno", '=', $tradeNo)->first();
        if ($trade != null) {
            exit("Success");
        } else {
...
private static function golangalipay_callback($request)
{
        //您在www.zfbjk.com的商户ID
    $alidirect_pid = Config::get("golangalipay_id");
        //您在www.zfbjk.com的商户密钥
    ...
    if (strtoupper(md5($alidirect_pid . $tradeNo . $Money . $title . $memo)) == strtoupper($Sign)) {
        ...
        echo $memo.$Sign . $tradeNo . $Money . $title;
        exit('Fail');
    }
}
public static function wxcallback($request) {
 Pay::golangwx_callback($request);
}
public static function alicallback($request) {
  Pay::golangalipay_callback($request);
}
